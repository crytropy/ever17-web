/**
 * Scene IR: the engine-facing intermediate representation of a KID-engine
 * scenario scene. Adapters (e.g. the Ever17 SC3 decompiler) lower original
 * bytecode into this shape; the runtime consumes only this. The runtime must
 * not need to know anything about the source format internals; everything not
 * yet understood is preserved as explicit `unknown` ops.
 */
...
export interface IrScene {
  /** IR_SCHEMA_VERSION at generation time (absent in pre-versioned output). */
