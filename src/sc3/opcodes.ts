...
export const OPCODES: ReadonlyMap<string, OpcodeDef> = new Map(
  Object.entries({
    ...
    "1020": def("EFFECT_ON", "E", "high", "screen effect by id, labeled by debug.scr effect test: 4=QUA1 5=QUA2 12=quake(params in vars 571-576) 18=sakura 26=rain(intensity var 569) 27=sunbeams(variant var 568) 32=filter2 41=snow 44=filter 45=blink 46=flash 47/48/49=map eyecatch/pins/route"),
    "1021": def("EFFECT_OFF", "E", "medium", "stops an effect by category id (filter(44) and filter2(32) both stop via 13; sunbeams via 7, rain via 6, snow via 14). Category mapping incomplete"),
    ...
    "1041": def("VIEWPORT_RECT", "EEEEE", "medium", "(x, y, w, h, frames): debug effect test zooms to (332,185,200x150) over 90 frames right after the effect-test banner; CG pan/zoom"),
